...
Pi Framework uses subagents to turn framework rules into repeatable delivery workflows.
...
They must resolve the conflict through the framework conflict-resolution rule.
...
The initial Pi Framework delivery topology is:
...
- semantic line breaks and UK English in framework-facing prose;
...
- read-only assessment against Pi Framework rules;
- scoring and prioritised improvement plans;
- identifying framework gaps before bootstrap or adoption work.

Does not own:

- changing files unless explicitly invoked for bootstrap work;
- treating AI Harness, ADRs, DBML, or language style as optional when the framework applies.
...
- project instructions conflict with Pi Framework rules;
- quality tooling is missing but required for the project risk;
- user requests would bypass TDD or canonical specs.

Escalation should include evidence and a recommended path, but must not silently choose a source of truth when the framework requires clarification.
...
- adopting Pi Framework in an existing repository;
- checking readiness;
- planning bootstrap work;
- auditing framework compliance without editing files.
