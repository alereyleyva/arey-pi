...
function sendWorkflow(pi: ExtensionAPI, args: string, ctx: { ui: { notify(message: string, level?: string): void }; isIdle(): boolean }, kind: string, usage: string) {
	if (!args.trim()) {
...
export default function areyPi(pi: ExtensionAPI) {
	pi.registerCommand("arey-doctor", {
		...
		handler: async (_args, ctx) => {
			...
			const prompts = commands.filter((command) => command.source === "prompt" && command.sourceInfo?.source?.includes("arey-pi"));
			const skills = commands.filter((command) => command.source === "skill" && command.sourceInfo?.source?.includes("arey-pi"));
			...
			pi.sendMessage({ customType: "arey-pi-doctor", content: report, display: true, details: {} });
		},
	});
	...
	pi.registerCommand("arey-bootstrap", {
		...
		handler: async (args, ctx) => {
			...
			pi.sendMessage({ customType: "arey-pi-bootstrap", content: report, display: true, details: result });
		},
	});
	...
	pi.registerCommand("arey-sync", {
		...
		handler: async (args, ctx) => sendWorkflow(pi, args || "the current repository", ctx, "sync", "Usage: /arey-sync [scope]"),
	});

	pi.registerCommand("arey-review", {
		description: "Run an Arey Pi adversarial engineering review",
		handler: async (args, ctx) => sendWorkflow(pi, args || "the current diff", ctx, "review", "Usage: /arey-review [scope]"),
	});

	pi.registerCommand("arey-assess", {
		description: "Assess project readiness against Arey Pi rules",
		handler: async (args, ctx) => sendWorkflow(pi, args || "the current repository", ctx, "assess", "Usage: /arey-assess [scope]"),
	});
}
